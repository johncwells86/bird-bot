...
# Log into your Walmart.com account, and add an item into your cart.
# In the chrome debugger, look for 
cookiestr = """<REPLACE WITH YOUR SESSION COOKIE FROM CHROME DEBUGGER>"""
...
class Walmart:
    ...
    def atc(self, offer_id):
        ...
        while True:
            ...
            try:
                r = self.session.post("https://www.walmart.com/api/v3/cart/:CRT/items", json=body, headers=headers)
                if r.ok:
                    self.status_signal.emit({"msg": "Added To Cart", "status": "carted"})
                    return
                else:
                    self.status_signal.emit({"msg": "Error Adding To Cart: {}".format(r.text), "status": "error"})
                    time.sleep(self.error_delay)
            except Exception as e:
    ...
    def check_cart_items(self):
        ...
        body = {"postalCode": profile["shipping_zipcode"], "city": profile["shipping_city"],
                ...
                "customerType:type": "", "affiliateInfo:com.wm.reflector": ""}
        while True:
            ...
            try:
                ...
                if r.ok:
                    self.status_signal.emit({"msg": "CART RESPONSE: {}".format(r.text), "status": "normal"})
            ...
            try:
                ...
                if r.status_code == 200:
                    try:
                        ...
                        pass
                self.status_signal.emit({"msg": "Error Submitting Shipping Method {}".format(r.text), "status": "error"})
                time.sleep(self.error_delay)
            except Exception as e:
            ...
            try:
              ...
              #  self.session.cookies = r.cookies
                ...
                if r.status_code == 200:
                    ...
                    return pi_hash
                self.status_signal.emit({"msg": "Error Submitting Payment {}".format(r.text), "status": "error"})
    ...
    def submit_billing(self, pi_hash):
        ...
        while True:
            ...
            try:
                r = self.session.post(
                    "https://www.walmart.com/api/checkout/v3/contract/:PCID/payment",
                    json=body,
                    headers=headers)
                #self.session.cookies = r.cookies
                ...
                if r.status_code == 200:
                    try:
                        ...
                        pass
                self.status_signal.emit({"msg": "Error Submitting Billing {}".format(r.text), "status": "error"})
                if self.check_browser():
                    return
                time.sleep(self.error_delay)
            except Exception as e:
